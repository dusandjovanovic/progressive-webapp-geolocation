		...
		redraw() {
			...
			const v =
				...
				Math.pow(
					...
					Math.max(
						0,
						Math.min(maxZoom - this.props.leaflet.map.getZoom(), 12)
					) / 2
				);
				...
				reduce(
					...
					(grid, point) => {
						...
						if (!cell) {
							...
							cell[0] =
								(cell[0] * cell[2] + p.x * k) / (cell[2] + k);
							cell[1] =
								(cell[1] * cell[2] + p.y * k) / (cell[2] + k);
							cell[2] += k;
							cell[3] += 1;
						}
			...
			const getDataForHeatmap = (points, leafletMap) =>
				roundResults(
					accumulateInGrid(points, leafletMap, getBounds(leafletMap))
				);
